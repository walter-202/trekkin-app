...
- **Estado real y brecha (15%):** primer plano, checkpoints, finalización, GPX, SQLite, `mapTrack` completo para el mapa, cámara free con fit inicial único, Opción A sin punto semilla y la guarda de finalización `origin === "free"` tienen evidencia pura. Correcciones 2026-09-24 (backlog `docs/plan/gps_recording_backlog.md`): sync sin `gpx.sha256: undefined` (`stripUndefined` antes de `setDoc`) y panel de diagnóstico GPS en vivo (`gpsStats`/`gpsEvents` + `diagBox` en `TrackingView`, banner con reintento). Pendientes: validar en Expo Go con el panel y la matriz de cámara, Firebase Emulator, Android/iOS con pantalla apagada/terminada/endurance, share sheet nativo y revisión UI completa.
- **Mapeo Técnico:**
  - _Dominio:_ `activity.schemas.ts`, `calculations.ts`, `src/core/domain/trackFormats.ts` (`toGeoJSON`, `buildGPX11`).
  - _Aplicación:_ `StartRecordingFromPlan`, `AddCheckpoint`, `RecordPoint` (`classifyPointDiscard`), `FinishActivity`, `ExportTrackFile`.
  - _Infraestructura:_ `activityService.ts` (+ `sanitizeDoc.ts`), `locationService.ts` (`RECORDING_WATCH_OPTIONS`), `useActivityStore.ts` (`mapTrack`, `gpsStats`/`gpsEvents`), `activityTrackDb.ts` (`loadTrackPoints`).
  - _Presentación:_ `RecordView` → `TrackingView` (track completo, fit free único y diag GPS) / `ResultView`, `AddCheckpointModal`.
  - _Suites:_ `src/tests/activity_hu8.test.ts`, `src/tests/map_hu8.test.ts`, `src/tests/activity_record_sqlite.test.ts`, `src/tests/activity_store.test.ts`, `src/tests/activity_store_sqlite.test.ts`, `src/tests/track_formats_hu7_hu8.test.ts`.
...
| **HU-08** | Grabar GPS y GPX       |   🟢 85%    | `activity_hu8.test.ts`, `map_hu8.test.ts`, `activity_record_sqlite.test.ts`, `activity_store.test.ts`, `activity_store_sqlite.test.ts`, `track_formats_hu7_hu8.test.ts`, `activity_track_db.test.ts`, `activity_gpx_storage.test.ts`, `background_location.test.ts` |
