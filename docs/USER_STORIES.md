...
- **Estado real y brecha (15%):** primer plano, checkpoints, finalización, GPX, SQLite y sincronización local tienen evidencia pura. Correcciones 2026-09-24 (backlog `docs/plan/gps_recording_backlog.md`): sync sin `gpx.sha256: undefined` (`stripUndefined` antes de `setDoc`) y panel de diagnóstico GPS en vivo (`gpsStats`/`gpsEvents` + `diagBox` en `TrackingView`, banner con reintento). Pendientes: validar en Expo Go con el panel (ver por qué se descartan fixes), Firebase Emulator, Android/iOS con pantalla apagada/terminada/endurance, share sheet nativo y revisión UI completa.
- **Mapeo Técnico:**
  ...
  - _Infraestructura:_ `activityService.ts` (+ `sanitizeDoc.ts`), `locationService.ts` (`RECORDING_WATCH_OPTIONS`), `useActivityStore.ts` (`gpsStats`/`gpsEvents`).
  - _Presentación:_ `RecordView` → `TrackingView` (diag GPS) / `ResultView`, `AddCheckpointModal`.
