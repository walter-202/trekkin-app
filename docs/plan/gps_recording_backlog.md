...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **P0-1** | Fix `gpx.sha256: undefined`: sanitizar metadata antes de `setDoc` (omitir campos `undefined`)                                                                                  | `activityService.ts` (`toCloudActivity` / `updateActivity`), `activityGpx.ts`, `useActivityStore.syncFinishedActivity` | Sync sin error de Firestore; `npm test` (suite `activity_gpx_storage`) verde        |
| **P0-2** | Fix error de inicio iOS: reintentos de fix + mensajes que distingan permiso vs fix vs store                                                                                    | `FreeRecordView.tsx`, `locationService.ts`                                                                             | En Expo Go iOS: iniciar grabación funciona o muestra error accionable con reintento |
| **P0-3** | Fix 0 m Android/iOS: portar `gpsStats`/`gpsEvents`/`diagBox` de cruz, verificar `startWatch` al entrar a Tracking, banner + retry si falla, asegurar `phase === "in_progress"` | `useActivityStore.ts`, `TrackingView.tsx`, `locationService.ts`                                                        | Caminando en Expo Go: distancia > 0 y el panel muestra motivos de descarte          |
