...
- **Foco Próximo Inmediato:**
  1. `BK-023` (HU-06): Migrar `TrackingView` al nuevo `TrekMap` nativo + alerta de desvío off-route >50m.
  2. `BK-010` / `BK-013` (HU-04): Gestor de descarga física de teselas a `expo-file-system` (`tileDownloader.ts`).
...
| :--- | :--- | :--- | :--- | :---: | :--- |
| **BK-001** | HU-03 | MapLibre Nativo dev-build | Build nativo C++/GPU. Pospuesto a V2 para no bloquear iteración rápida de devs en Expo Go. | 🔄 **V2** | P2 · 1d |
| **BK-002** | HU-03 | Configuración y capas libres | OpenStreetMap libre vía `<UrlTile />` (Android) y Apple Maps nativo (iOS) sin API keys pagas de Google. | ✅ **HECHO** | P0 · 0.5d |
| **BK-003** | HU-03 | `TrekMap.tsx` nativo | Componente unificado sobre `react-native-maps`: Polyline, marcadores inicio/fin/checkpoints, bounds dinámico, 0×403. | ✅ **HECHO** | P0 · 2d |
| **BK-004** | HU-03 | Retirar lastre y catálogo liviano | `RouteCard` consume `coverImageUrl` o fotos de usuario con 0 requests de mapa; reemplazo de `PlanMap` en `RouteDetailView`. | ✅ **HECHO** | P0 · 0.5d |
...
| ID | HU | Tarea | Qué cambia | Estado | Pri / Esf |
| :--- | :--- | :--- | :--- | :---: | :--- |
| **BK-010** | HU-04 | Casos `CreatePack/DeletePack` | Gestor de descarga de teselas por cuadrante a `expo-file-system` (`/offline_packs/{routeId}/`). | 🟡 **EN CURSO** | P0 · 2d |
...
| **BK-013** | HU-04 | Modo avión en mapa | `TrekMap` soporta prop `offlinePackPath` (`file://...`) para leer teselas locales sin conexión. | 🟡 **EN CURSO** | P1 · 1d |
...
| **BK-023** | HU-06 | Tracking sobre `TrekMap` | Migrar `TrackingView` a `TrekMap`, follow-me, snap to polyline, alerta off-route >50m. | ⏳ **PENDIENTE** | P1 · 2d |
...
| :--- | :--- | :--- | :--- | :---: |
| **BK-040** | HU-03 | Migrar `Detail` a `TrekMap` | `RouteDetailView` migrado a `TrekMap` nativo y botón de descarga offline habilitado. | ✅ **HECHO** |
...
```
1. BK-023 (HU-06: Migrar TrackingView a TrekMap + alerta desvío >50m)
2. BK-010 / BK-013 (HU-04: tileDownloader.ts a expo-file-system)
