...
## 3c. Stock crítico por OEM (HU-07, RF-11/RF-12)

- `ListCriticalStockGroups` (`:domain`) valida **RBAC en dominio** (solo rol `admin` vía
  `SessionManager.currentSession()`; lanza `InventoryError.SoloAdmin` si no), filtra los
  grupos con `esCritico = stockTotal <= reorderPoint` y ordena por stock ascendente
  (los más urgentes primero). La UI solo oculta la entrada (defensa en profundidad).
- `InventoryRepositoryImpl` (`:data`) implementa `stockAgrupadoPorOem()` reutilizando
  `CatalogDao.getAllOemParts` + `findVariantsByOemIds` y `InventoryDao.stockByVariantIds`
